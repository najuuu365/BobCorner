...
import { RotateCcw, Award, BookOpen, Hash, Sparkles, Undo } from 'lucide-react';
  ...
  const move = useCallback(
    (direction: 'UP' | 'DOWN' | 'LEFT' | 'RIGHT') => {
      ...
      for (let r = 0; r < 4; r++) {
        ...
        if (newRow.some((val, idx) => val !== currentGrid[r][idx])) {
          changed = true;
        }
      }
    ...
    <div className="space-y-6 max-w-md mx-auto">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <button
          onClick={handleUndo}
          disabled={!prevBoard}
          className="p-2.5 rounded-2xl border border-haven-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col items-center justify-center disabled:opacity-40 transition-all hover:bg-haven-50"
        >
